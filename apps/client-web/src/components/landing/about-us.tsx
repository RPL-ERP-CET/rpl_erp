import { Card, CardContent } from "@client-web/components/ui/card";

type AboutUsContent = {
    title: string;
    description: string;
    image: string;
};

const ABOUT_US_CONTENT = [
    {
        title: "Ayiranallur Estate",
        description:
            "Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatum ex beatae illo veritatis, delectus ab, quas tempora magni quis dolorem ut velit rerum minus officia harum nisi deleniti. Ab, magni!",
        image: "https://rplkerala.com/wp-content/uploads/2019/02/kulathupuzaestate-300x167.jpg",
    },
    {
        title: "Kulathupuzha Estate",
        description:
            "Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatum ex beatae illo veritatis, delectus ab, quas tempora magni quis dolorem ut velit rerum minus officia harum nisi deleniti. Ab, magni!",
        image: "https://rplkerala.com/wp-content/uploads/2019/02/kulathupuzaest001-300x167.jpg",
    },
    {
        title: "Factories",
        description:
            "Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatum ex beatae illo veritatis, delectus ab, quas tempora magni quis dolorem ut velit rerum minus officia harum nisi deleniti. Ab, magni!",
        image: "https://rplkerala.com/wp-content/uploads/2019/02/factory001-300x167.jpg",
    },
];

const AboutUsCard = (content: AboutUsContent) => {
    return (
        <Card>
            <CardContent>
                <h2>{content.title}</h2>
                <p>{content.description}</p>
            </CardContent>
        </Card>
    );
};

export default function AboutUs() {
    return <section className="bg-white"></section>;
}
